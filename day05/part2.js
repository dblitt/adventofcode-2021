...
for (let i = 0; i < stuff.length; i++) {
    ...
    if (mode === 'h') {
        ...
        let smallerX, biggerX, smallerY, biggerY;
        if (stuff[i][0][1] > stuff[i][1][1]) {
            biggerY = stuff[i][0][1];
            smallerY = stuff[i][1][1];
        } else {
        ...
        for (let j = smallerY; j <= biggerY; j++) {
            let xValue;
            if ((stuff[i][0][1] > stuff[i][1][1]) !== (stuff[i][0][0] > stuff[i][1][0])) { // if inverted diag line (XOR)
                xValue = biggerX - (j - smallerY);
            } else {
                xValue = smallerX + (j - smallerY);
            }
            // console.log(xValue)
            // grid[j] must be at least xValue long
            while (grid[j].length <= xValue) {
